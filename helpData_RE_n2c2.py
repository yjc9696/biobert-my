...
class DataHelper(object):
    ...
    def get_sens_and_tags_and_entsRel(self, datas, case=0):
        ...
        with open(data_root + branch, 'w') as f:
            ...
            f.write(BIO_data)
        # if case > 1:
        #     testdf = pd.read_csv(data_root + branch, sep="\t")
        #     if len(testdf) != all_rel_num:
        #         print(str(len(testdf)) + "???" + str(all_rel_num))
        #     with open(data_root + base, 'w') as f:
        #         f.write('sentence' + '\t' + 'label' + "\n")
        #         for i in range(len(testdf))
        #             f.write(testdf['sentence'][i] + '\t' + testdf['label'][i] + '\n')
